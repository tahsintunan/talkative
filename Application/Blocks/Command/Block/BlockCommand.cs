...
public class BlockCommandHandler : IRequestHandler<BlockCommand>
{
    ...
    public BlockCommandHandler(IUserService userService)
    {
        _userService = userService;
    }
    public async Task<Unit> Handle(BlockCommand request, CancellationToken cancellationToken)
    {
        if (request.Blocked == request.BlockedBy) return Unit.Value;
        ...
        if (blockedUser == null || blockedByUser == null) return Unit.Value; 
        
        
        // blockedBy
        var blockedByList = blockedUser.BlockedBy;
        if (blockedByList == null) blockedByList = new List<string?>();
        ...
        if (blockedList == null) blockedList = new List<string?>();
